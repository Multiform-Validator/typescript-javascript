...
describe("validateSurname", () => {
  it("validates surname with correct length", () => {
    const result: ValidateFunctions = validateSurname("Jackson", {
      minLength: 3,
      maxLength: 25,
    });
  ...
  it("returns error for surname with numbers", () => {
    const result: ValidateFunctions = validateSurname("Jack5on");
    expect(result).toEqual({
      isValid: false,
  ...
  it("returns error for surname with special characters", () => {
    const result: ValidateFunctions = validateSurname("Jack$on");
    expect(result).toEqual({
      isValid: false,
  ...
  it("returns error for invalid surname", () => {
    const result: ValidateFunctions = validateSurname("Ja", {
      minLength: 3,
      maxLength: 25,
    });
    expect(result).toEqual({
  ...
  it("should return false for surnames that are too short", () => {
    const result: ValidateFunctions = validateSurname("J", {
      minLength: 2,
      maxLength: 20,
    });
    expect(result.isValid).toBe(false);
  ...
  it("Should return true for a valid surname Jackson, Johnson, or Smith", () => {
    const result1: ValidateFunctions = validateSurname("Jackson");
    const result2: ValidateFunctions = validateSurname("Johnson");
    const result3: ValidateFunctions = validateSurname("Smith");
