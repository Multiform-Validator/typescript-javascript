...
describe("isDecimal", () => {
	...
	it("should throw error when the input is Infinity", () => {
		expect(() => isDecimal(Infinity as any)).toThrow(
	...
	it("should throw error when the input is a URL", () => {
		expect(() => isDecimal(new URL("https://example.com") as any)).toThrow(
			errorToThrow,
		);
	});
	...
	it("should throw error when the input is a DataView", () => {
		expect(() => isDecimal(new DataView(new ArrayBuffer(2)) as any)).toThrow(
			errorToThrow,
		);
	});
	...
	it("should throw error when the input is a boolean", () => {
		expect(() => isDecimal(true as any)).toThrow(
