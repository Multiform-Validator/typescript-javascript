...
describe("validateEmail", () => {
  it("should throw an error if the input is not a string", () => {
    expect(() => validateEmail(123 as unknown as string)).toThrow(TypeError);
  });

  it("should return defaultErrorMsg when errorMsg is null", () => {
    const result: ValidateFunctions = validateEmail("foo@bar.com", {
  ...
  it("should validate an corret email length", () => {
    const result: ValidateFunctions = validateEmail(
      "test@testaaaaaaaaaaaaae.com",
      {
        maxLength: 15,
      },
    );
  ...
  it("should invalidate an email with a non-allowed domain", () => {
    const result: ValidateFunctions = validateEmail("test@notallowed.com", {
  ...
  it("should validate an email with a custom allowed domain", () => {
    const result: ValidateFunctions = validateEmail("test@mydomain.com", {
  ...
  it("should throw an error when errorMsg is not an array or null", () => {
    expect(() =>
      validateEmail("jor@dio.com", { errorMsg: 123 as unknown as string[] }),
  ...
  it("should throw an error if any element of the errorMsg array is different from string or null", () => {
    expect(() =>
      validateEmail("jd@dio.com", { errorMsg: [123 as unknown as string] }),
  ...
  it("should validate an email that ends with the country code", () => {
    const result: ValidateFunctions = validateEmail("test@gmail.com.us", {
      country: "us",
    });
    expect(result.isValid).toBe(true);
    expect(result.errorMsg).toBe(null);
  ...
  it("should validate email and return the custom error message and defaultMsgErrors too", () => {
    const result: ValidateFunctions = validateEmail("1@gmail.com", {
